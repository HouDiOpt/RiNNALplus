...
function obj = subsref(var_obj, ind)
    ...
    elseif (num_dim == 2)
        ...
        if max(ind.subs{1})<=var_obj.blkorg{2} && max(ind.subs{2})<=var_obj.blkorg{3}
            ...
            idx_row_temp = 0.5*((idx_j_temp-1).*(idx_j_temp)) + idx_i_temp;
            idx_col_temp = 1:num_constr; 
            info.Operator_Matrix{var_obj.block_no} = sparse(idx_row_temp,idx_col_temp,value_temp,dim_temp,num_constr);
